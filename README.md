...
Async get, `model('asyncProp', callbackFunc)`, is not supported anymore. *All* 2 argument calls are handled by
the setter, allowing defining new computed properties on the fly. If you need async values, use
...
Following literate tests are processed by [jsmd](https://github.com/vesln/jsmd) &mdash; `npm test`

Test coverage reports are generated by [istanbul](https://gotwarlost.github.io/istanbul/) &mdash; `npm run coverage`
